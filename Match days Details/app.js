...
getMainDiv.appendChild(dataBody);

for(var key in rounds){          //creating matchday boxes
    var matchDaysBox = document.createElement("div");
    matchDaysBox.setAttribute("class","matchday-box");
    matchDaysBox.setAttribute("onClick", `details(${key})`);
    matchDaysBox.appendChild(document.createTextNode(rounds[key].name));
    dataBody.appendChild(matchDaysBox);
}

var getModalDiv = document.getElementById("main-modal");
var detailsDiv = document.createElement("div");
detailsDiv.setAttribute("id","detail-box");
getModalDiv.appendChild(detailsDiv);
...
function details(el) {
    getModalDiv.style.display = "flex";
...
//=============================================
    ...
    trHeadings.setAttribute("id" , "table-headings")
    var tableHeadings = ["date", "team 1", "team 2","team-1 score", "team-2 score"]
    for(var i=0; i<5; i++) {
        var tdOfHead = document.createElement("td");
        tdOfHead.appendChild(document.createTextNode(tableHeadings[i].toUpperCase()));
        trHeadings.appendChild(tdOfHead);
    }
    tbl.appendChild(trHeadings);
    detailsBody.appendChild(tbl);
//====================================================
    for(var key2 in rounds[el].matches){ //loop on selected round with the help of(el)

        var selectedRound = rounds[el].matches[key2];
        //console.log(selectedRound)

        var trProperties = document.createElement("tr");
        tbl.appendChild(trProperties)

        for(var key3 in selectedRound) {

        if(typeof(selectedRound[key3]) === "object") {
            var tdd1 = document.createElement("td");
            tdd1.appendChild(document.createTextNode(selectedRound.score.ft[0]));
            trProperties.appendChild(tdd1);

            var tdd2 = document.createElement("td");
            tdd2.appendChild(document.createTextNode(selectedRound.score.ft[1]));
            trProperties.appendChild(tdd2);

    }else{
        var tdd = document.createElement("td");
        tdd.appendChild(document.createTextNode(selectedRound[key3]));
        trProperties.appendChild(tdd);
    }
        }
    }
}
